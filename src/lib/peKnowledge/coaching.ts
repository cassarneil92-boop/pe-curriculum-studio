...
import { TPSR_MASTER_PE_ENTRY } from "./tpsrMaster";
import {
...
import {
  buildSecPeOptionPlanningInsights,
  buildSecQualityInsights,
  buildSchemeSecPETips,
} from "./secPeOptionEngines";
import { SEC_PE_OPTION_MASTER_PE_ENTRY } from "./secPeOptionMaster";
import {
  buildSportCurriculumPlanningInsights,
  buildSportQualityInsights,
  buildSchemeSportTips,
} from "./sportCurriculumEngines";
import { SPORT_CURRICULUM_MASTER_PE_ENTRY } from "./sportCurriculumMaster";
...
export interface TPSRQualityReview {
  score: number;
  band: string;
  checks: { label: string; met: boolean }[];
  warnings: string[];
...
export interface LearningScienceQualityReview {
  score: number;
  band: string;
  checks: { label: string; met: boolean }[];
  warnings: string[];
...
export function getPlanningAssistantKnowledgeSuggestions(
  ...
  const tpsrInsights = buildTPSRPlanningInsights(prompt, {
    yearGroup: context.yearGroup,
    topicId: context.topicId,
    activityArea: context.activityArea,
    lessonAim: context.lessonAim ?? prompt,
  ...
  const secInsights = buildSecPeOptionPlanningInsights(prompt, {
    yearGroup: context.yearGroup,
    topicId: context.topicId,
    activityArea: context.activityArea,
    lessonAim: context.lessonAim ?? prompt,
  ...
  const sportInsights = buildSportCurriculumPlanningInsights(prompt, {
    yearGroup: context.yearGroup,
    topicId: context.topicId,
  ...
  if (secInsights.length > 0) {
    const secCard: PEKnowledgeCardViewModel = {
      entry: SEC_PE_OPTION_MASTER_PE_ENTRY,
      reason: secInsights[0],
      planningPrompts: secInsights.slice(0, 3),
      assessmentPrompt: "Include exam-style questions and retrieval practice for theory topics.",
      differentiationPrompt: "Tiered exam questions — describe, explain, apply with scaffolds.",
    };
    return [secCard, ...cards].slice(0, limit);
  }
  if (sportInsights.length > 0) {
    const sportCard: PEKnowledgeCardViewModel = {
      entry: SPORT_CURRICULUM_MASTER_PE_ENTRY,
      reason: sportInsights[0],
      planningPrompts: sportInsights.slice(0, 3),
      assessmentPrompt: "Assess technique and tactical decision-making in game phases.",
      differentiationPrompt: "Adapt space, equipment, or rules — not just remove challenge.",
    };
    return [sportCard, ...cards].slice(0, limit);
  }
  ...
  if (tpsrInsights.length > 0) {
    const tpsrCard: PEKnowledgeCardViewModel = {
      entry: TPSR_MASTER_PE_ENTRY,
      reason: tpsrInsights[0],
      planningPrompts: tpsrInsights.slice(0, 3),
      assessmentPrompt: "Observe responsibility evidence against today's level focus.",
      differentiationPrompt: "Match responsibility level to pupil readiness — respect before leadership.",
    };
    return [tpsrCard, ...cards].slice(0, limit);
  }
  if (lsInsights.length > 0) {
    const lsCard: PEKnowledgeCardViewModel = {
      entry: LEARNING_SCIENCE_MASTER_PE_ENTRY,
      reason: lsInsights[0],
      planningPrompts: lsInsights.slice(0, 3),
      assessmentPrompt: "Plan retrieval check and delayed evidence against WILF.",
...
export function buildSchemeProgressionCoachReport(
  ...
  const secPeTips = buildSchemeSecPETips(scheme);
  const sportPeTips = buildSchemeSportTips(scheme);
...
export function buildKnowledgeQualityInsights(
  ...
  const tpsrInsights = buildTPSRQualityInsights(lesson);
  const clReflectionWarned = clInsights.some((i) => i.message.includes("reflection"));
  for (const insight of tpsrInsights) {
    if (clReflectionWarned && insight.message.includes("reflection") && insight.id !== "tpsr-review") {
      continue;
    }
  ...
  for (const si of buildSecQualityInsights(lesson)) {
    insights.push({
      id: si.id,
      area: si.area,
      message: si.message,
  ...
  for (const spi of buildSportQualityInsights(lesson)) {
    insights.push({
      id: spi.id,
      area: spi.area,
      message: spi.message,
...
export function buildTPSRQualityReviewForLesson(
  lesson: LessonBuilderFormData
): TPSRQualityReview {
...
export function buildLearningScienceQualityReviewForLesson(
  lesson: LessonBuilderFormData
