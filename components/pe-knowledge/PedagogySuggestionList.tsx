...
import { PEKnowledgeCard } from "@/components/pe-knowledge/PEKnowledgeCard";
    ...
    <Card className="border-teal-100/70 bg-teal-50/20">
      ...
      <div className="space-y-2">
        {suggestions.map((card, index) => (
          <PEKnowledgeCard key={card.entry.id} card={card} defaultOpen={index === 0} />
        ))}
      </div>
      <p className="mt-3 text-[10px] text-slate-500">
        Advisory guidance from the internal PE knowledge base — not a substitute for professional judgement.
      </p>
    </Card>
  );
}
